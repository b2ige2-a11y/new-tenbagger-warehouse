...
const featuredProfiles: Partial<Record<string, Pick<Stock, "description" | "kpis" | "bullCase" | "bearCase" | "risks" | "catalysts" | "quickFacts" | "businessSegments" | "whyItMatters" | "supplyChain">>> = {
  nvidia: {
    ...
    quickFacts: [{ label: "무엇을 하나", value: "AI 연산용 GPU 플랫폼을 만듭니다" }, { label: "핵심 키워드", value: "GPU · CUDA · 네트워킹 · HBM" }, { label: "같이 볼 테마", value: "AI 인프라 · 반도체" }],
    ...
    kpis: [{ label: "데이터센터 매출", description: "AI 인프라 수요가 실제 기업 매출로 이어지는지 보는 중심 지표입니다.", importance: "high" }, { label: "매출총이익률", description: "제품 구성과 가격 결정력, 비용 구조가 어떻게 바뀌는지 보여 줍니다.", importance: "high" }, { label: "네트워킹 매출", description: "GPU 외 연결 장비 수요가 함께 커지는지 확인하는 단서입니다." }, { label: "재고", description: "공급과 실제 고객 수요 사이에 과도한 간극이 없는지 점검합니다." }, { label: "하이퍼스케일러 CAPEX", description: "대형 클라우드 기업의 설비투자 계획은 인프라 수요의 외부 관찰 포인트입니다." }],
  ...
  amd: {
    ...
    quickFacts: [{ label: "무엇을 하나", value: "CPU와 GPU를 설계하는 반도체 기업" }, { label: "핵심 키워드", value: "서버 CPU · AI GPU · PC · 콘솔" }, { label: "같이 볼 테마", value: "AI 인프라 · 반도체" }],
    ...
    kpis: [{ label: "데이터센터 부문 흐름", description: "서버 CPU와 AI 가속기 관련 수요가 실제 사업에 미치는 영향을 점검합니다.", importance: "high" }, { label: "서버 CPU 채택", description: "클라우드·기업 고객에서의 플랫폼 채택 변화가 중요한 관찰 포인트입니다.", importance: "high" }, { label: "AI GPU 채택", description: "AI 가속기 제품이 고객 시스템에 얼마나 적용되는지 확인합니다." }, { label: "PC 수요", description: "PC 교체 주기와 채널 재고는 클라이언트 사업에 영향을 줄 수 있습니다." }, { label: "매출총이익률", description: "제품 구성과 데이터센터 비중 변화가 수익성에 주는 영향을 봅니다." }],
  ...
  tsmc: {
    ...
    quickFacts: [{ label: "무엇을 하나", value: "고객이 설계한 반도체를 위탁 생산합니다" }, { label: "핵심 키워드", value: "파운드리 · 선단 공정 · CoWoS · 가동률" }, { label: "같이 볼 테마", value: "반도체 · HBM/메모리" }],
    ...
    kpis: [{ label: "선단 공정 수요", description: "고성능 칩 고객이 최신 공정을 얼마나 필요로 하는지 확인합니다.", importance: "high" }, { label: "첨단 패키징 공급 능력", description: "AI 칩과 HBM을 연결하는 패키징이 공급 병목이 되는지 살펴봅니다.", importance: "high" }, { label: "가동률", description: "공장 설비가 실제 주문에 따라 어느 정도 활용되는지 보여 주는 지표입니다." }, { label: "CAPEX", description: "공정과 생산능력 확대를 위한 장기 투자 방향을 확인합니다." }, { label: "고객 구성", description: "특정 고객·최종 시장에 대한 의존도와 수요 분산을 점검합니다." }],
